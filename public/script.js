...
const headerObserverCallback = function (entries, observer) {
  entries.forEach((entry) => {
    if (entry.isIntersecting) navigation.classList.remove("navigation--fixed");
    if (!entry.isIntersecting) navigation.classList.add("navigation--fixed");
  });
};
...
const btnParentComponent = document.querySelectorAll("tertiary-btn-parent");

const btnEffectCallback = function (entries, observer) {
  entries.forEach((entry) => {
    if (entry.isIntersecting)
      entry.target.querySelector(".btn--tertiary").classList.add("btn--active");
    if (!entry.isIntersecting)
      entry.target
